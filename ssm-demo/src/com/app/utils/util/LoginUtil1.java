 ...
 * @date 2013-1-6
 */
public class LoginUtil1 {
	private static final Logger log = Logger.getLogger(LoginUtil.class);

	private final static String KEY_FOR_USER = "!$%^@abcd#$#@wkmsUser";
	 ...
	 * @return String
	 */
	public static String getLoginUserMobile(HttpServletRequest request) {
		String loginstr = getLoginInfo(request);
		if (null != loginstr && !"".equals(loginstr)) {
			String[] logininfo = loginstr.split("\\++");
			if (logininfo.length >= 7) {
	 ...
	 * @author caiqf
	 * @date 2013-1-6 上午11:05:57
	 * @describe 获取登陆用户是否为主帐号
	 * @return String
	 */
	public static String getLoginUserIsmajor(HttpServletRequest request) {
		String loginstr = getLoginInfo(request);
		if (null != loginstr && !"".equals(loginstr)) {
			String[] logininfo = loginstr.split("\\++");
			if (logininfo.length >= 7) {
	 ...
	 * @author caiqf
	 * @date 2013-1-6 上午11:05:57
	 * @describe 获取登陆用户用户主帐号编号
	 * @return String
	 */
	public static Integer getLoginUserMajorId(HttpServletRequest request) {
		String loginstr = getLoginInfo(request);
		if (null != loginstr && !"".equals(loginstr)) {
			String[] logininfo = loginstr.split("\\++");
			if (logininfo.length >= 7) {
				return new Integer(logininfo[5]);
			}
		}
		return null;
	}

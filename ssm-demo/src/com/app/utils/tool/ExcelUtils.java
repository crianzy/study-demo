...
public class ExcelUtils {
	 ...
	 * @version 1.0
	 */
	public static void writeExcelOld(String[] title, String filepath,
	...
	public static void writeExcel(String[] title, String filepath,
		...
		try {
			...
			int row = 1;
			// 添加第一个工作表并设置第一个Sheet的名字
				WritableSheet sheet = wwb.createSheet(sheetname+sheetNum, 0);
			...
			for (int i = 0; i < list.size(); i++) {
				...
				for (int j = 0; j < obj.length; j++) {
					...
					// 超过5000条数据就再建sheet，这里设置为3用于测试
					if (row > 5000) {
						sheetNum++;
						sheet = wwb.createSheet(sheetname + sheetNum, sheetNum);
						row = 1;
						
						// 填充标题行数据
						for (int f = 0; f < title.length; f++) {
							WritableFont titleWf = new WritableFont(
									WritableFont.createFont("仿宋_GB2312"),// 字体
									WritableFont.DEFAULT_POINT_SIZE, // 字号
									WritableFont.BOLD, // 粗体
									...
									Colour.BLACK, // 字体颜色
									ScriptStyle.NORMAL_SCRIPT);
							WritableCellFormat wcabc = new WritableCellFormat(titleWf);
							wcabc.setAlignment(Alignment.CENTRE); // 设置文字水平居中
							wcabc.setVerticalAlignment(VerticalAlignment.CENTRE); // 设置文字垂直居中
							wcabc.setBorder(Border.ALL, BorderLineStyle.THIN); // 设置边框线
							wcabc.setBackground(jxl.format.Colour.ICE_BLUE); // 设置单元格背景颜色
							label = new Label(f, 0, title[f], wcabc);
							sheet.addCell(label);
							sheet.setColumnView(f, 25); // 设置列的宽度
							sheet.setRowView(0, 400); // 设置行的高度
						} 
					...
					label = new Label(j, row,  getObjString(obj[j]),wc);
					try {
						if (label != null)
							sheet.addCell(label);

...
public class JsonUtil {
	...
	public static Map getMapFromJsonObjStr(String jsonObjStr, Class clazz) {
		...
		for (Iterator iter = jsonObject.keys(); iter.hasNext();) {
			...
			map.put(key, JSONObject
					.toBean(jsonObject.getJSONObject(key), clazz));
		}
		return map;
	}
	...
	public static String getJsonStrFromXML(String xml) {
		...
		// JSONArray jsonText = (JSONArray) xmlSerializer.read(xml);
		// return jsonText.toString();
		return String.valueOf(xmlSerializer.read(xml));
	}
...
class JsonDateValueProcessor implements JsonValueProcessor {
 ...
 /**
  * 将string 转换json 转义特殊字符
  * @param s
  * @return
  * @author 汤建东
  * @date 2012-11-19 下午5:34:47
  *
  */
